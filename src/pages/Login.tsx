...
const Login = () => {
  ...
  const { slug } = useParams<{ slug: string }>(); // 🔹 pega empresa da URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setError("");
      setLoading(true);
      await signIn(email, password, slug!); // 🔹 agora login respeita empresa
      toast({
        title: "Login realizado com sucesso",
        description: "Você foi conectado à sua conta",
      });
      navigate(`/${slug}/admin`); // 🔹 redireciona para admin da empresa logada
    } catch (error: any) {
      setError(error.message || "Falha ao fazer login. Verifique seus dados.");
    } finally {
      setLoading(false);
    }
  };
